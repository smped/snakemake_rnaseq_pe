...
ALL_FQC = expand(["data/{step}/FastQC/{sample}{reads}_fastqc.{suffix}"],
                 suffix = ['zip', 'html'],
                 reads = [r1, r2],
                 sample = samples['sample'],
                 step = ['raw', 'trimmed'])
ALL_TRIMMED = expand(["data/trimmed/fastq/{sample}{reads}" + ext],
                      reads = [r1, r2],
                      sample = samples['sample'])
ALL_ALN = expand(["data/aligned/bam/{sample}/Aligned.sortedByCoord.out.bam"],
                 sample = samples['sample'])
ALL_OUTPUTS = []
